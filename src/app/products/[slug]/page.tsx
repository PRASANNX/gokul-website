...
import { getSingleProductCheckoutUrl, getWholesaleCheckoutUrl, UserContactDetails } from "@/lib/whatsapp";
...
export default function ProductDetailPage({ params }: Props) {
  ...
  const handleDirectCheckout = (details: UserContactDetails) => {
    const url = getSingleProductCheckoutUrl(product.name, selectedWeight.weight, selectedWeight.price, quantity, lang, details);
                  ...
                  {!isCheckingOut ? (
                    <div className="grid grid-cols-2 gap-3">
                      <button 
                        ...
                        className="btn-commerce btn-primary !py-5 w-full !text-[13px] md:!text-[14px] !tracking-[0.1em] shadow-xl hover:-translate-y-1 transition-all overflow-hidden relative"
                      >
                        <span className={`absolute inset-0 flex items-center justify-center transition-all duration-300 ${added ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'} text-[#4ade80] gap-2 font-black`}>
                          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>
                          {messages.product.added}
                        </span>
                        <span className={`flex items-center justify-center gap-2 transition-all duration-300 ${added ? '-translate-y-full opacity-0' : 'translate-y-0 opacity-100'}`}>
                          <svg className="w-5 h-5 hidden xl:block" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>
                      ...
                      <button 
                        ...
                        className="btn-commerce btn-whatsapp !py-5 w-full !text-[13px] md:!text-[14px] !tracking-[0.1em] shadow-xl hover:-translate-y-1 transition-transform flex items-center justify-center gap-2"
